...
function map:on_started()
  ...
  if game:get_value("kakariko_shop_bow") then
    ...
    if shop_heart ~= nil then
      shop_heart:set_position(96, 64)
    end
    if shop_small_magic_flask ~= nil then
      shop_small_magic_flask:set_position(144, 64)
    end
    if shop_big_magic_flask ~= nil then
      shop_big_magic_flask:set_position(192, 64)
    end
    if shop_arrow ~= nil then
      shop_arrow:set_position(240, 64)
    end
    if shop_bomb ~= nil then
      shop_bomb:set_position(288, 64)
    end
  else
